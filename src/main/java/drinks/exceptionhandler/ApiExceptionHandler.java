package drinks.exceptionhandler;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		var message = "O corpo da requisição está inválido.";
		var apiError = new ApiError(status.value(), message, OffsetDateTime.now());
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		var apiError = new ApiError(status.value(), message, OffsetDateTime.now(), fields);
		return new ResponseEntity<>(apiError, headers, status);
	}

	@ExceptionHandler(ResourceNotFoundException.class)
	private ResponseEntity<ApiError> handleResourceNotFoundException(ResourceNotFoundException ex, WebRequest request) {
		var apiError = new ApiError(HttpStatus.NOT_FOUND.value(), ex.getMessage(), OffsetDateTime.now());
	...
	private ResponseEntity<ApiError> handleIllegalArgumentException(IllegalArgumentException ex, WebRequest request) {
		...
		var apiError = new ApiError(status.value(), ex.getMessage(), OffsetDateTime.now());
